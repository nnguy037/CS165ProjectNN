...
//	SSL_CTX_set_options(ctx, SSL_OP_ALL | SSL_OP_NO_SSLv2 | SSL_OP_SINGLE_DH_USE);
	...
	//BIO_new(BIO_s_mem());
	BIO *binfile = BIO_new(BIO_s_mem()); //creates new bio
	//BIO_write
	BIO_write(binfile,buf,BUFFER_SIZE); //copy the challenge received into new BIO stuff
	//BIO_new(BIO_f_md());
	BIO *hash; //same as simple.cpp
	hash = BIO_new(BIO_f_md()); //same as simple.cpp
	//BIO_set_md;
	BIO_set_md(hash, EVP_sha1());//same as simple.cpp
	//BIO_push;
	BIO_push(hash, binfile);//same as simple.cpp
	//BIO_gets;
	char Wabuffet[EVP_MAX_MD_SIZE];
   	int mdlen = BIO_gets(hash, Wabuffet, EVP_MAX_MD_SIZE);;
	string hash_string = buff2hex((const unsigned char*)Wabuffet, mdlen);
    ...
    //PEM_read_bio_RSAPrivateKey (part C from simple.cpp
	char privateKey[] = "rsaprivatekey.pem"; //from simple.cpp
	//char publicKey[] = "rsapublickey.pem";

	BIO *privKey;//, *pubKey;
	privKey = BIO_new_file(privateKey, "r"); //from simple.cpp
	//pubKey = BIO_new_file(publicKey, "r") ;

	RSA *rsa = PEM_read_bio_RSAPrivateKey(privKey, NULL, NULL, NULL); //from simple.cpp

    //RSA_private_encrypt (Part D from simple.cpp)

	unsigned char buff_two[128];
	memset(buff_two, 0, sizeof(buff_two));
	

    	int siglen = RSA_private_encrypt(117, (const unsigned char*)Wabuffet, buff_two, rsa, RSA_PKCS1_PADDING); //RSA_PKCS1_PADDING - 11;
    	char* signature= (char*)buff_two;
	...
	BIO_flush(binfile);
	//SSL_write
	SSL_write(ssl, signature, BUFFER_SIZE);
	...
	//BIO_flush
	//BIO_new_file
	BIO *newBIOFile = BIO_new_file(file,"r");
	char writeTo[BUFFER_SIZE];
	memset(writeTo,0,sizeof(writeTo));
	//BIO_puts(server, "fnf");
	int bytesSent=0;
	int fileOpen = 1;
	int temp = 0;
	while(fileOpen > 0)
	{
		...
		fileOpen = BIO_read(newBIOFile, writeTo, 1);//read 1 character at a time to avoid grabbing too many bytes
	//SSL_write(ssl, buffer, bytesRead);
		temp =SSL_write(ssl, writeTo, 1); //sends 1 character over into ssl as well as update if file is open
		//cout << " " << temp << " ";
		bytesSent = bytesSent + fileOpen;
	}
